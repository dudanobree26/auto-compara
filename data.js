...
let CARROS = []; // preenchido pela planilha

async function carregarPlanilha() {
  const URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vTfJg8Zq8CHvHtv15QZ252Ydvey0aiXt6h7mbxLthgneWB-oMPEiVl9x5Pp-zbzRZA2RHqgX36z9Py2/pub?gid=1782389322&single=true&output=csv";
  try {
    const res   = await fetch(URL);
    const texto = await res.text();
    const linhas = texto.trim().split("\n").slice(1); // pula cabeçalho

    CARROS = linhas.map(linha => {
      const c = linha.split(",");
      return {
